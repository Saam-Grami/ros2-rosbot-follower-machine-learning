...
def generate_launch_description():
    ...
    pkg_share = FindPackageShare('mtre4820_rosbot')

    # Fix mesh paths
    import subprocess
    pkg_path = subprocess.getoutput('ros2 pkg prefix mtre4820_rosbot') + '/share/mtre4820_rosbot'
    os.environ['GAZEBO_MODEL_PATH'] = (
        pkg_path
        + ':' + pkg_path + '/meshes'
        + ':' + os.environ.get('GAZEBO_MODEL_PATH', '')
    ...
    robot_description = {
        'robot_description': launch.substitutions.Command(
            ['xacro ', PathJoinSubstitution([pkg_share, 'urdf', 'rosbot.xacro'])]
        )
    }
